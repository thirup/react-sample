...
var PaginationList = require('./pagination/PaginationList');
var ToolBar = require('./toolbar/ToolBar');
var TableFilter = require('./TableFilter');
var {TableDataStore} = require('./store/TableDataStore');
...
class BootstrapTable extends React.Component{
  ...
  constructor(props) {
    ...
    if(this.props.cellEdit){
      ...
      this.props.cellEdit.__onCompleteEditItems__ = this.handleEditItems.bind(this);
  ...
  render(){
    ...
    var columns = this.props.children.map(function(column, i){
      return {
        ...
        index: i,
        isLocalData: column.props.isLocalData,
        keyName: column.props.keyName,
        keyValue: column.props.keyValue,
        url: column.props.url,
        comboData: column.props.comboData,
        inputType: column.props.inputType,
  ...
  handleEditField(newVal, rowIndex, fieldName){
    let result = this.store.edit(newVal, rowIndex, fieldName).get();
    this.setState({
      data: result
    });
  ...
  //Still testin
  handleEditItems(editedItems){
    var that = this;
    editedItems.forEach(function(editedItem) {
      if (editedItem.fieldName) {
        let result = that.store.edit(editedItem.value, editedItem.rowIndex, editedItem.fieldName).get();
        if(that.props.cellEdit.afterSaveCell){
          that.props.cellEdit.afterSaveCell(this.state.data[editedItem.rowIndex], editedItem.fieldName, editedItem.value);
        }
        that.setState({
          data: result
        });
      } else {
        that.props.cellEdit.__onCompleteEdit__(editedItem.value, editedItem.rowIndex, editedItem.colIndex);
      }
    });
