...
var TableEditColumn = require('./TableEditColumn');
...
class TableBody extends React.Component{
  ...
  render(){
    ...
    var tableRows = this.props.data.map(function(data, r){
      var tableColumns = this.props.columns.map(function(column, i){
        ...
        if (column.inputType == 'combobox') {
          displayValue = data[column.displayName];
        }
        ...
        if(this.editing &&
          ...
          this.state.currEditCell.cid == i) {
            return(
              <TableEditColumn completeEdit={this.handleCompleteEditCell.bind(this)}
                               completeEditItems={this.handleCompleteEditItems.bind(this)}
                               key={i}
          ...
          if(typeof column.format !== "undefined"){
            var formattedValue = column.format(displayValue, data);
            ...
            return(
              <TableColumn dataAlign={column.align}
                           ...
                           onEdit={this.handleEditCell.bind(this)}>{displayValue}</TableColumn>
            )
          }
        }
      }, this);
  ...
  handleCompleteEditItems(editedItems){
    this.setState({currEditCell: null});
    //if(null != newVal)
      this.props.cellEdit.__onCompleteEditItems__(editedItems);
  }
